...
export class MatClockView<D> implements AfterContentInit {
  ...
  _handleTouchStart(event: any) {
    ...
    const _handleTouchEnd = () => {
      ...
      event.target.removeEventListener('touchend', _handleTouchEnd);
    };
    ...
    event.target.addEventListener('touchend', _handleTouchEnd);
  }
  ...
  _init() {
    ...
    if (this.twelveHour) {
      ...
      for (let i = 0; i < hourNames.length / 2; i++) {
        ...
        this._hours.push({
          value: i,
          displayValue: i === 0 ? '0' : hourNames[i],
  ...
  private setTime(event: any) {
    ...
    //       2;
    const minOuterRadius = width < height ? width : height;
    const outer =
      this.hourView &&
      z >
        (minOuterRadius * (CLOCK_OUTER_RADIUS / 100) +
          minOuterRadius * (CLOCK_INNER_RADIUS / 100)) /
          2;
